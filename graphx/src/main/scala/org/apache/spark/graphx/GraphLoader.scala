...
object GraphLoader extends Logging {
  ...
  /**
    * Loads a sampled graph from an edge list formatted file where each line contains
    * two integers:  a source id and a target id. Skips lines that begin with `#`.
    * Sampling process keeps an edge with given probability.
    *
    * If desired the edges can be automatically oriented in the positive
    * direction (source Id < target Id) by setting `canonicalOrientation` to
    * true.
    ...
    * @param sc SparkContext
    * @param path the path to the file (e.g., /home/data/file or hdfs://file)
    * @param probability probability to keep an edge while loading
    * @param canonicalOrientation whether to orient edges in the positive
    *        direction
    * @param numEdgePartitions the number of partitions for the edge RDD
    * Setting this value to -1 will use the default parallelism.
  ...
  : Graph[Int, Int] =
  {
    ...
    val edges = lines.mapPartitionsWithIndex { (pid, iter) =>
      ...
      val builder = new EdgePartitionBuilder[Int, Int]
      iter.foreach { line =>
        if (!line.isEmpty && line(0) != '#' && rng.nextDouble() < probability) {
          val lineArray = line.split("\\s+")
          if (lineArray.length < 2) {
            throw new IllegalArgumentException("Invalid line: " + line)
          }
